...
void AD_Init(void)
{
    ...
    ADC_RegularChannelConfig(ADC1, ADC_Channel_0, 1,ADC_SampleTime_55Cycles5);  // 通道0放在序列1的位置
    ...
    ADC_InitStructure.ADC_ContinuousConvMode = ENABLE; // DISABLE单次扫描 ENABLE连续扫描
    ADC_InitStructure.ADC_ScanConvMode = ENABLE;    // 开启扫描模式
    ADC_InitStructure.ADC_NbrOfChannel = 4;         // 一共有4个序列
    ...
    DMA_InitStructure.DMA_Mode = DMA_Mode_Circular;           // 是否启用自动重装器
...
// void AD_GetValue(void)   // 指定通道，返回该通道的值
// { 
