...
char Serial_RxPacket[100];      // 接收暂存区 不超过100个字符
...
void USART1_IRQHandler(void)
{
    ...
    if (USART_GetITStatus(USART1, USART_IT_RXNE) == SET)
    {
        ...
        if (RxState == 0)
        {
            if (RxData == '@' && Serial_RxFlag == 0)        // 防止数据包传输过快读取时被覆盖，这里&&限制该数据包读取条件 防止跳过数据包
            {
                RxState = 1;
                pRxState = 0;
            }
        ...
        else if (RxState == 2)
        {
            ...
            {
                RxState = 0;
                Serial_RxPacket[pRxState] = '\0';   // 补充字符串结束符 程序识别字符串发送结束
                Serial_RxFlag = 1;
            }
        }
